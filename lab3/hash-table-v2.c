...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		errno = pthread_mutex_init(&(entry->lock), NULL);
                             ...
                             uint32_t value)
{
	...
	uint32_t errno = 0;
	errno = pthread_mutex_lock(&(hash_table_entry->lock));
	if (errno != 0) exit(errno);
	...
	if (list_entry != NULL) {
		...
		errno = pthread_mutex_unlock(&(hash_table_entry->lock));
		if (errno != 0) exit(errno);
		return;
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		errno = pthread_mutex_destroy(&(entry->lock));
		if (errno != 0) exit(errno);
	}
