...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	uint32_t errno = 0;
	errno = pthread_mutex_init(&(hash_table->lock), NULL);
...
{
	uint32_t errno = 0;
	errno = pthread_mutex_lock(&(hash_table->lock));
	if (errno != 0) exit(errno);
	...
	if (list_entry != NULL) {
		...
		errno = pthread_mutex_unlock(&(hash_table->lock));
		if (errno != 0) exit(errno);
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	uint32_t errno = 0;
	errno = pthread_mutex_destroy(&(hash_table->lock));
	if (errno != 0) exit(errno);
